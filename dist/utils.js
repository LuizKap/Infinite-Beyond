...
export function isGamesResponse(value) {
...
export function favoriteHandler(target) {
    ...
    if (target.classList.contains('favorite-icon')) {
        ...
        const backgroundImg = card?.querySelector('.cards-img');
        if (!name || !target.id) {
            alert('I couldnt add your game to favorites :(');
            return;
        }
        if (target.dataset.clicked === 'false') {
            let favorite = { id: parseInt(target.id), name: name, background_image: backgroundImg.src };
...
export function animateGenres() {
    const sidebar = document.querySelector('#genres-sidebar');
    const genre_button = document.querySelector('#genres-button');
